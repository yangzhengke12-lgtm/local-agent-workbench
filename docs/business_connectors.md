...
- 当前本地 Git 工作区摘要，包括未提交文件、diff 统计、暂存区统计和当天提交标题，用来支持“今天本地改了什么”“写项目日报”这类请求。

注意：这里的群聊上下文只来自飞书实际投递到 webhook 的事件，不是完整群历史读取。运行时状态保存在本地 `feishu_events.json`，并已被 `.gitignore` 忽略。本地 Git 摘要是只读的，不会自动注入完整 diff 或文件内容。
...
When creating a task, the backend injects recent text messages from the same `chat_id`, a same-day summary from `agent_tasks.json`, and a read-only local Git workspace summary. This supports requests such as "based on the messages above", "summarize what we did today", and "summarize today's local file changes."

Important boundary: chat context is built only from events Feishu delivered to the webhook. It is not full group-history access. Runtime state lives in local `feishu_events.json`, which is ignored by git. The local Git summary includes status, diff stats, staged diff stats, and same-day commit titles, not full diffs or file contents.
...
- Not yet supported: encrypted event payloads via `FEISHU_EVENT_ENCRYPT_KEY`, full group-history fetching, full-diff injection, interactive cards, document events, native Feishu slash commands, or user identity permission mapping.
