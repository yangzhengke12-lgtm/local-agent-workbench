...
[![Tests](https://img.shields.io/badge/tests-226%20passed-green)](.)
[![Python](https://img.shields.io/badge/python-3.12-blue)](.)
[![FastAPI](https://img.shields.io/badge/backend-FastAPI-009688)](.)
[![Electron](https://img.shields.io/badge/desktop-Electron-47848F)](.)

> 当前定位：本地项目工作台。已包含数据库、内部 API 和飞书群通知的最小 connector demo；联网搜索、企业 SSO、安装包和更深的 Jira/GitLab/飞书双向应用适合作为后续 tool/provider adapter 接入。
...
| **工程化质量** | `runtime/` 模块化拆分，无 runtime 到 manager 的反向依赖，226 个自动化测试 |
...
仓库已包含最小可运行业务连接器 demo：只读 SQLite 数据库查询、内部 API 白名单请求和飞书自定义机器人通知，见 [docs/business_connectors.md](docs/business_connectors.md)。
...
- 企业认证 / SSO
- 打包安装器
- 飞书入站 Bot 事件、Jira/GitLab 写入适配器或其他更深的公司系统适配器
...
| **Engineering hygiene** | Modular `runtime/` package, no runtime-to-manager reverse dependency, 226 automated tests |
...
The repository also includes minimal runnable connector demos for read-only SQLite business queries, allowlisted internal API calls, and Feishu/Lark custom bot notifications: [docs/business_connectors.md](docs/business_connectors.md).
...
- enterprise auth / SSO
- packaged installer
- inbound Feishu bot events, Jira/GitLab write adapters, or other deeper company-system adapters
